...
@section('content')
	<div class="question_detail" style="margin: 10px;">
		...
		<p class="post_info">
			...
			<a href="{{URL::to('profile/'. $question->User->id )}}">
		...
		<div class="question_content">
			...
			@if($question->user_id == Auth::user()->id)
				<a href='{{ URL::to('question/edit/' . $question->id) }}'>
					Bewerk je vraag
				</a>
			@endif
		</div>
		...
		@foreach($question->answers as $answer)
			...
			{{-- Check if the question is anwered --}}
			...
			<p>{!! Markdown::convertToHtml(HTML::entities($answer['content'])) !!}</p>
				<p class="post_info">
					...
					<a href="{{URL::to('profile/'. $answer->User->id )}}">
						{{$answer->User->username}}
					</a><br><br>
				</p>
			<!-- Edit button voor de eigenaar van het antwoord. -->
			@if(Auth::user()->id == $answer->user_id)
				<a href='{{ URL::to('answer/edit/' . $answer->id) }}'>
					<i class="fa fa-pencil"></i>&nbsp Bewerk je antwoord
				</a><br><br>
			@endif
			<a href="{{ action('CommentController@getCreate', [$question->id, $answer->id]) }}">
			...
			@foreach($answer->comments as $comment)
				<div class="answer" style="margin: 10px;"> 
					...
					<p class="post_info">
						...
						<a href="{{URL::to('profile/'. $comment->User->id )}}">
							{{$comment->User->username}}
						</a><br><br>
					</p>
					<p class="question_content">{{$comment->content}}</p>
					@if($comment->user_id == Auth::user()->id)
						<a href='{{ action('CommentController@getEdit', [$question->id, $answer->id, $comment->id]) }}'>
							Bewerken
						</a>
					@endif
				</div>
